...
// Obter keywords
$keywords = filter_input(INPUT_POST, 's', FILTER_SANITIZE_STRING);
...
if (!empty($keywords)) {
    ...
    if ($num > 0) {
        ...
        $html .= '<tr><td colspan="9">Sem registos</td></tr>';
    }
    $html .= '</tbody></table>';
} else {
    $html = '<div>Deve definir um termo de pesquisa. '
            . '<a href="?m='.$module.'&a=read" class="btn btn-primary">Voltar</a></div>';
}
echo $html;